...
__kernel void clahe_3D(
    ...
    // Determine the tile region for the current pixel
    int tile_x = global_x / tileSize;
    int tile_y = global_y / tileSize;
    int tile_z = global_z / tileSize;

    // Calculate the start and end coordinates of the tile
    int x_start = tile_x * tileSize;
    int y_start = tile_y * tileSize;
    int z_start = tile_z * tileSize;
